...
void bx_instr_lin_access(unsigned cpu, bx_address lin, bx_address phy,
  ...
  fuzz_hook_memory_access(cpu, phy, len, memtype, rw, dataptr);
}
...
void bx_instr_cnear_branch_taken(unsigned cpu, bx_address branch_eip, bx_address new_eip) {
    fuzz_instr_cnear_branch_taken(cpu, branch_eip, new_eip);
}
void bx_instr_cnear_branch_not_taken(unsigned cpu, bx_address branch_eip) {
    fuzz_instr_cnear_branch_not_taken(cpu, branch_eip);
}
void bx_instr_ucnear_branch(unsigned cpu, unsigned what, bx_address branch_eip, bx_address new_eip) {
    fuzz_instr_ucnear_branch(cpu, what, branch_eip, new_eip);
}
void bx_instr_far_branch(unsigned cpu, unsigned what, Bit16u prev_cs, bx_address prev_eip, Bit16u new_cs, bx_address new_eip) {
    fuzz_instr_far_branch(cpu, what, prev_cs, prev_eip, new_cs, new_eip);
}
...
void bx_instr_before_execution(unsigned cpu, bxInstruction_c *i) {
    fuzz_instr_before_execution(cpu, i);
}
