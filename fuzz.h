...
struct sym_addr_t {
	...
	inline bool operator<(const sym_addr_t &rhs) const {
		return addr < rhs.addr || (addr == rhs.addr && pid < rhs.pid);
	}
};
...
void fuzz_inject_vmcall(uint64_t rcx, uint64_t r8, const void *xmm0,
			const void *xmm3);
...
void fuzz_instr_ucnear_branch(unsigned cpu, unsigned what,
			      bx_address branch_rip, bx_address new_rip);
...
extern bool fuzz_unhealthy_input;
extern bool fuzz_do_not_continue;
...
void enum_handle_ept_gap(unsigned int gap_reason, bx_address gap_start,
			 bx_address gap_end);
...
void load_regions(std::map<uint16_t, uint16_t> &pio_regions,
...
unsigned long sym_to_addr(std::string bin, std::string name, bool full = false,
			  int pid = 0);
...
void handle_breakpoints_func_call(unsigned cpu, bx_address func,
				  bx_address rip);
